<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-thin h3">Maestro de Articulos</h1>
</div>
      ...
      <div class="panel panel-default">
        ...
        <div style="margin-bottom: 25px;">
          <table class="table m-b-none" ui-jq="footable" data-filter="#filter" data-page-size="5">
            ...
            <tbody>
              <?php if(!empty($articulos_general)): ?>
                <?php foreach ($articulos_general as $articulo) : ?>
                  <tr>
                      <td><?php echo $articulo->ID; ?></td>
                      <td><?php echo $articulo->ABREVIATURA; ?></td>
                      
                      <td><?php echo $articulo->DESCRIPCION; ?></td>
                      <td data-value="1">
                        <span class="label bg-<?php echo $articulo->ESTADO == 0 ? 'danger' : 'success'; ?>" title="Active">
                          <?php echo $articulo->ESTADO == 0 ? 'Desabilitado' : 'Activo'; ?>
                        </span>
                      </td>
                      <td>


                        <a href="#edit" onclick="edit(<?php echo  $articulo->ID ?>);"  title="Editar" class="btn btn-default">
                        ...
                        <?php
                        if($tarifa->ESTADO == '0'){ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $articulo->ID ?>/1');" title="Activar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Activar
                            </a>
                        <?php }else{ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $articulo->ID ?>/0');" title="Desactivar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Desactivar
                            </a>
                        <?php }?>
...
<script type="text/javascript">
    function metod_confirm (id) {
        ...
        if (r == true) {
            window.location = "<?php echo base_url(); ?>articulo/dashboard/delete/"+id;
        } else {
            //Cancelado
        }
    }
    function edit(id){
        ...
        $("#content-ajax #content").load("<?php echo base_url(); ?>articulo/dashboard/articulo_edit/"+id);
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
    function create(){
        $("#content-ajax #content").load("<?php echo base_url(); ?>articulo/dashboard/create_articulo");
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
