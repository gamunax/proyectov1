...
class Front_model extends CI_Model {
    ...
    public function provincias($idDepa)
    {
     ...
     public function _lst_proyectos(){
        $this->db->select('idinmueble, titulo, idTipoInmueble, idUrbanizacion, y.descripcion as desdistrito, banos, cocheras_p, cocheras_p, dormitorios, areaConstruida, substring(descripcionEspanol,1,130) as descripcionEspanol, substring(descripcionIngles,1,130) as descripcionIngles, idTipoMoneda, activo, imagen1, imagen2, imagen3, imagen4, imagen5, imagen6, imagen7, estado, idcodinmueble, idgradoconservacion, precio, idconservacion, title');
        $this->db->from('inmueble');
        $this->db->join('distrito y', 'inmueble.idUrbanizacion = y.id','left');            
           ...
           $this->db->select('idinmueble, titulo, idTipoInmueble, idUrbanizacion, y.descripcion as desdistrito, banos, cocheras_p, cocheras_p, dormitorios, areaConstruida, descripcionEspanol, idTipoMoneda, activo, imagen1, imagen2, imagen3, imagen4, imagen5, imagen6, estado, idcodinmueble, idgradoconservacion, precio, idconservacion, title, descripcionIngles');
            ...
            if($descripcion == '%'){
               null;
                ...
                $this->db->where('descripcionIngles', $descripcion);
            }
             if($caracteristica == '%'){
               null;
            }else{
