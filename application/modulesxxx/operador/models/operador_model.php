...
class Operador_model extends CI_Model {
	...
	public function list_operadores(){
		$query = $this->db->query("SELECT CONCAT(APELLIDO_PATERNO, ' ', APELLIDO_MATERNO) AS APELLIDOS, NOMBRES, CELULAR, COD_OPERADOR, DNI, ESTADO, TIPO_OPERADOR FROM MA_OPERADOR");
		return $query->result();		
		/*$this->db->select('*');
