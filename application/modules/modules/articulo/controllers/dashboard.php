...
class Dashboard extends MX_Controller
{
    ...
    public function cu_create_articulo(){
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            if(!empty($id)){
                ...
                */
                ...
                //if(is_array($imagen)){
                 $upd = $this->articulos->update_articulo($arr_datos,(int)$id);
                 if($upd) {
                ...
                redirect('articulo/dashboard/','refresh');
            }else{
                $ins_datos = $this->articulos->_insert_articulos($arr_datos);
                if($ins_datos){
    ...
    public function delete($id,$tipo = null){
        ...
        $this->articulos->update_articulo($additional_data,$id);
        // $this->session->set_flashdata('message', 'delete');
        redirect(base_url().'articulo/dashboard','refresh');
    }

    public function articulo_edit($id = NULL){
        if($this->session->userdata('perfil') == FALSE || $this->session->userdata('perfil') != 'admin')
        {
            $this->session->set_flashdata('usuario_incorrecto', 'sesion_login');
            redirect('login/inicio','refresh');
        ...
        $dt_articulo = $this->articulos->_obtener_datos_articulos((int)$id);
        if(count($dt_articulo)!=0){
            ## Formulario
            ...
            ##
            $data['estado']      = $dt_articulo['ESTADO'];
            $data['id']          = $dt_articulo['ID'];
