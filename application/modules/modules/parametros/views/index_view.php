<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-thin h3">Maestro de Parametros</h1>
</div>
      ...
      <div class="panel panel-default">
        ...
        <div style="margin-bottom: 25px;">
          <table class="table m-b-none" ui-jq="footable" data-filter="#filter" data-page-size="12">
            ...
            <tbody>
              <?php if(!empty($parametros_general)): ?>
                <?php foreach ($parametros_general as $parametro):?>
                  <tr>
                      <td><?php echo $parametro->TABLA; ?></td>
                      <td><?php echo $parametro->CODIGO; ?></td>
                      
                      <td><?php echo $parametro->DESCRIPCION; ?></td>
                      <td data-value="1">
                        <span class="label bg-<?php echo $parametro->ESTADO == 0 ? 'danger' : 'success'; ?>" title="Active">
                          <?php echo $parametro->ESTADO == 0 ? 'Desabilitado' : 'Activo'; ?>
                        </span>
                      </td>
                      <td>


                        <a href="#edit" onclick="edit('<?php echo  $parametro->TABLA ?>', '<?php echo $parametro->CODIGO ?>');"  title="Editar" class="btn btn-default">
                        ...
                        if($parametro->ESTADO == '0'){ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $parametro->TABLA ?>', '<?php echo  $parametro->CODIGO ?>/1');" title="Activar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Activar
                            </a>
                        <?php }else{ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $parametro->TABLA ?>','<?php echo  $parametro->CODIGO ?>/0');" title="Desactivar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Desactivar
                            </a>
                        <?php }?>
...
<script type="text/javascript">
    function metod_confirm (tabla, codigo) {
        var r = confirm("¿Desea realizar esta operación?");
        if (r == true) {
            window.location = "<?php echo base_url(); ?>parametros/dashboard/delete/"+tabla+'/'+codigo;
        } else {
            //Cancelado
        }
    }
    function edit(tabla, codigo){
        $("#content-ajax #content").html("");
        $("#content-ajax #content").load("<?php echo base_url(); ?>parametros/dashboard/parametro_edit/"+tabla+'/'+codigo);
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
    function create(){
        $("#content-ajax #content").load("<?php echo base_url(); ?>parametros/dashboard/create_parametro");
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
