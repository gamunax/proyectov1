...
class Dashboard extends MX_Controller
{
    public function __construct(){
        parent::__construct();
        $this->load->model('remito_ca_model', 'remitosca', TRUE);
    }
     ...
     public function index()
    {
        ...
        $data['remitos_general'] = $this->remitosca->list_remitosca();
        ## Inicio de Sesión
        $data['page_title'] = 'Dashboard';

        ## Template Admin Dashboard
        $data['module'] = 'remito_ca';
    ...
    public function create_remito(){
       ...
       // $data['idcategoria'] = 0;
        ...
        ## Template Admin Dashboard
        $data['module'] = 'remito_ca';
        $data['view_file'] = 'remito_ca_frm_view';
        //echo Modules::run('template/header_login',$data);
    ...
    public function cu_create_remito(){
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            $arr_datos = array(
                'ID'      =>  $id,
                'EMISION'    =>  $emision,
                'REMITO_INICIAL'    =>  $reminicial,
                'REMITO_FINAL'        =>  $remfinal,
                'ESTADO'        =>  $estado,
            ...
            if(!empty($id)){
                ...
                */
                ...
                //if(is_array($imagen)){
                 $upd = $this->remitosca->update_remito($arr_datos,(int)$id);
                 if($upd) {
                ...
                $ins_datos = $this->remitosca->_insert_remitos($arr_datos);
                if($ins_datos){
                ...
                redirect('remito_ca/dashboard/','refresh');
            }
        }else{
            $this->session->set_flashdata('message', 'error');
    ...
    public function delete($id,$tipo = null){
        ...
        // $this->session->set_flashdata('message', 'delete');
        redirect(base_url().'remito_ca/dashboard','refresh');
    }
    ...
    public function remito_edit($id = NULL){
        ...
        $dt_remito = $this->remitosca->_obtener_datos_remitos((int)$id);
        if(count($dt_remito)!=0){
           ...
           $data['reminicial'] = array(
                ...
                'type'  => 'text',
                'required' => 'required',
                'value' => $dt_remito['REMITO_INICIAL']
            );
            $data['remfinal'] = array(
                'name'  => 'remfinal',
                'id'    => 'remfinal',
