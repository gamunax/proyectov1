...
class Liquidacion_model extends CI_Model {
    ...
    public function buscar_remito($emision, $remito){
       ...
       /* $this->db->select('*');
        ...
        return $arrayResultado;
        */
        $query = $this->db->query("SELECT a.EMISION, CASE WHEN a.REMITO_CA > 0 THEN a.REMITO_CA ELSE a.REMITO END REMITO, c.CONSIGNADO, c.DIRECCION_ENTREGA AS DIRECCION, a.GLOSA, a.ESTADO AS IDESTADO, d.DESCRIPCION AS ESTADO, a.ID_MOTIVO AS MOTIVO FROM di_asignaciones a, envio c, ma_parametros d WHERE a.emision = c.emision and a.remito = c.remito and d.tabla= 'estado' and a.estado = d.codigo  and c.emision = '$emision' and case when c.remito_ca > 0 then c.remito_ca else c.remito end = $remito  and a.NROVECES in (select max(NROVECES) from di_asignaciones b where a.emision = b.emision and a.remito = b.REMITO)");
        return $query->result();
     ...
     public function buscarliquidacion($fecha, $salida, $operador){
        ...
        $query = $this->db->query("SELECT a.EMISION, CASE WHEN a.REMITO_CA > 0 THEN a.REMITO_CA ELSE a.REMITO END REMITO, c.CONSIGNADO, c.DIRECCION_ENTREGA AS DIRECCION, a.GLOSA, a.ESTADO AS IDESTADO, d.DESCRIPCION AS ESTADO, a.ID_MOTIVO AS MOTIVO FROM di_asignaciones a, envio c, ma_parametros d WHERE a.emision = c.emision and a.remito = c.remito and d.tabla= 'estado' and a.estado = d.codigo  and DATE_FORMAT(a.FECHA_SALIDA,'%Y-%m-%d') = '$fecha' and a.salida = $salida and a.ID_OPERADOR = $operador and NROVECES in (SELECT MAX(nroveces) FROM di_asignaciones b,ma_operador o WHERE a.emision = b.emision AND a.remito = b.remito and b.id_operador = o.id and o.tipo_operador <> 'I') order by a.emision, a.remito");
        return $query->result();
