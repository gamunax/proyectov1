...
class Inmuebles_model extends CI_Model {
    ...
    public function lst_alquiler(){
        ...
        $query = $this->db->query("SELECT idinmueble, a.descripcion as departamento,  d.descripcion as destipoinmueble, b.descripcion as provincia, c.descripcion as distrito, titulo, inmueble.idTipoInmueble, idUrbanizacion,  banos, cocheras_p, cocheras_p, dormitorios, areaConstruida, descripcionEspanol, idTipoMoneda, activo, imagen1, imagen2, imagen3, imagen4, imagen5, imagen6, estado, idcodinmueble, idgradoconservacion, idcaracteristica, precio, idconservacion, imagen7, title, descripcionEspanol, idcodinmuebleingles FROM inmueble, distrito a, distrito b, distrito c, tipoinmueble d WHERE substr(inmueble.idUrbanizacion,1,4) = a.id  and substr(inmueble.idUrbanizacion,1,5) = b.id  and substr(inmueble.idUrbanizacion,1,7) = c.id and d.idTipoInmueble = inmueble.idTipoInmueble and inmueble.idcodinmueble = 'Alquiler' order by inmueble.idinmueble");
        return $query->result();
    ...
    public function _lst_cbo_caracteristicas(){
        $qry='SELECT idcaracteristica , descripcion, descripcionIngles FROM caracteristicas ORDER BY descripcion desc';
        $result=$this->db->query($qry);
        $conservacion['']='Seleccione caracteristica';
        foreach($result->result() as $row)
        {
            $conservacion[$row->idcaracteristica]=($row->descripcion);
        };
        $result->free_result();
        return $conservacion;
    }
