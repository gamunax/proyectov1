<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-thin h3">Maestro de Tarifas</h1>
</div>
      ...
      <div class="panel panel-default">
        ...
        <div style="margin-bottom: 25px;">
          <table class="table m-b-none" ui-jq="footable" data-filter="#filter" data-page-size="12">
            ...
            <tbody>
              <?php if(!empty($tarifas_general)): ?>
                <?php foreach ($tarifas_general as $tarifa) : ?>
                  <tr>
                      <td><?php echo $tarifa->CODIGO; ?></td>
                      
                      <td><?php echo $tarifa->DESCRIPCION; ?></td>
                      <td><?php echo $tarifa->TFA_BASE; ?></td>
                      <td><?php echo $tarifa->TFA_EXCESO; ?></td>
                      <td data-value="1">
                        <span class="label bg-<?php echo $tarifa->ESTADO == 0 ? 'danger' : 'success'; ?>" title="Active">
                          <?php echo $tarifa->ESTADO == 0 ? 'Desabilitado' : 'Activo'; ?>
                        </span>
                      </td>
                      <td>


                        <a href="#edit" onclick="edit(<?php echo  $tarifa->ID ?>);"  title="Editar" class="btn btn-default">
                        ...
                        <?php
                        if($tarifa->ESTADO == '0'){ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $tarifa->ID ?>/1');" title="Activar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Activar
                            </a>
                        <?php }else{ ?>
                            <a href="#" onclick="return metod_confirm('<?php echo  $tarifa->ID ?>/0');" title="Desactivar" class="btn btn-default">
                                <i class="fa fa-times-circle-o"></i>
                                Desactivar
                            </a>
                        <?php }?>
...
<script type="text/javascript">
    function metod_confirm (id) {
        ...
        if (r == true) {
            window.location = "<?php echo base_url(); ?>tarifa/dashboard/delete/"+id;
        } else {
            //Cancelado
        }
    }
    function edit(id){
        ...
        $("#content-ajax #content").load("<?php echo base_url(); ?>tarifa/dashboard/tarifa_edit/"+id);
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
    function create(){
        $("#content-ajax #content").load("<?php echo base_url(); ?>tarifa/dashboard/create_tarifa");
        $("#content-ajax").removeClass("hidden");
        $("#content-ajax").addClass("block");
    }
