...
class Dashboard extends MX_Controller
{
    public function __construct(){
        parent::__construct();
        $this->load->model('orden_servicio_model', 'orden_servicios', TRUE);
    }
     ...
     public function index()
    {
        ...
        ## Inicio de Sesión
        $data['page_title'] = 'Listado de Orden de Servicio';

        ## Template Admin Dashboard
        $data['module'] = 'orden_servicio';
    ...
    public function buscar_cliente($documento){
        $data = $this->orden_servicios->buscar_cliente($documento);
        echo json_encode($data);
    }

    public function buscar_os(){
        ...
        $data = $this->orden_servicios->return_os($fechaini, $fechafin, $estado, $documento);
        echo json_encode($data);
    }

    public function mostraros($id){
        $data = $this->orden_servicios->mostraros($id);
        echo json_encode($data);
    }

     public function os_insertar(){
       ...
       // $fecha_actual = date('d/m/Y',strtotime($fecha_actual));
        ...
        foreach ($array as $value) {
               ...
               // $remito    = $dt_remito['REMITO_INI'] + 1;
                $arr_datos = array(
                'EMI_ORDEN'      =>  $value['emision'],
                'NRO_ORDEN' =>  $value['norden'],
                'DOCUMENTO'    =>  $value['rucdni'],
                'NROCLIENTE'     =>  $value['nrocliente'],
                'ID_CLIENTE'    =>  0,
                'FECHA_ORDEN'   =>   $value['fechareg'],
                'GLOSA'        =>  $value['glosa'],
                'ESTADO'    =>  $value['ordenestado'],
                'TIPO_SERVICIO'     =>  $value['tiposervicio'],
                'CANT_ENVIOS' => $value['cantidad']
            ...
            if(!empty($id)){
                 $upd = $this->orden_servicios->update_orden_servicio($arr_datos,(int)$id);
                 if($upd) {
                ...
                $ins_datos = $this->orden_servicios->_insert_orden_servicio($arr_datos);
               // $os = (string)$os . (string)$ins_datos . "-";
                if($ins_datos){
    ...
    public function buscar_cliente_paquete($id){
        $data = $this->orden_servicios->buscar_cliente_paquete($id);
        echo json_encode($data);
    }
    ...
    public function buscar_servicio(){
        $data = $this->orden_servicios->buscar_servicio();
        echo json_encode($data);
    }

    public function buscar_tarifa(){
        $data = $this->orden_servicios->buscar_tarifa();
        echo json_encode($data);
    }

     public function buscar_tarifa_id($id){
        $data = $this->orden_servicios->buscar_tarifa_id($id);
        echo json_encode($data);
    }

    public function buscar_articulo(){
        $data = $this->orden_servicios->buscar_articulo();
        echo json_encode($data);
    }
     ...
     public function buscar_destino(){
        if($_GET['value'] != ""){
            $destino = $_GET['value'];
            $data = $this->orden_servicios->buscar_destino($destino);
            echo json_encode($data);
        }
    }

    public function buscar_paquete_tarifa_det($id_paquete, $id_zona){
        $data = $this->orden_servicios->buscar_paquete_tarifario_det($id_paquete, $id_zona);
        echo json_encode($data);
    }
    

    // public function orden_servicio_insertar(){
    ...
    //     //$nuevafecha = strtotime ( '-1 day' , strtotime ( $fecha_actual ) ) ;
    //     //$fecha_actual = date ( 'Y-m-j' , $nuevafecha );
    //     $dt_remito = $this->orden_servicios->generar_remito_lima(40);
    //     $emision   = $dt_remito['EMISION'];
    //     $remitos = $emision . "#";
    //     foreach ($array as $value) {
                
    //             $ope = $this->orden_servicios->mostraridoperador(3);
    ...
    //     echo json_encode($data);
    //     ## redirect('producto/repuestos/producto_edit/'.$ins_datos,'refresh');
    //     //redirect('orden_servicio/dashboard/','refresh');

    // }

    public function create_orden_servicio(){
        if($this->session->userdata('perfil') == FALSE || $this->session->userdata('perfil') != 'admin')
        {
            $this->session->set_flashdata('usuario_incorrecto', 'sesion_login');
            redirect('login/inicio','refresh');
       ...
       // $data['idcategoria'] = 0;
        ...
        $data['page_title'] = 'REGISTRAR orden_servicio';
        ## Template Admin Dashboard
        $data['module'] = 'orden_servicio';
        $data['view_file'] = 'index_view';
        echo Modules::run('template/header_login',$data);
